...
-- mostra todas as linhas de todas as colunas para primeiro contato com os dados
select * from portfolio;

-- cria tabela transcript
...
-- adiciona coluna copia de transactions vazia
alter table transcript add column copia_transactions varchar(100);
-- copia dados de transaction para a coluna criada
...
-- pega apenas um ou mais digitos após a palavra reward:, converte para inteiro e adiciona na nova coluna reward
...
select sum( case when idade < 18 then 1 else 0 end ) as "menores de idade",
       ...
       sum (case when idade>=40 and idade < 60 then 1 else 0 end ) as "adultos",  
       sum(case when idade>=60 and idade<118 then 1 else 0 end) as "idosos" from profile;
...
select gênero, 
       sum(case when idade>=18 and idade<40 then 1 else 0 end) as "jovens",
       sum (case when idade>=40 and idade < 60 then 1 else 0 end ) as "adultos",  
       sum(case when idade>=60 and idade<118 then 1 else 0 end) as "idosos" from profile group by gênero;
...
select portfolio.oferta as "oferta vista e completa", 100*sum(case when profile.idade
 ...
 from transcript group by transcript.id_oferta, person) my_table 
...
group by portfolio.oferta;
-- 
-- ofertas sem view e completadas
select portfolio.oferta AS "oferta não vista e completa", sum(case when profile.idade
        <40 then 1 else 0 end) as " total de jovens", sum(case when profile.idade>=40 and profile.idade<60 then 1 else 0 end) as "total de adultos", sum(case when profile.idade>=60 and profile.idade<118 then 1 else 0 end) as "total de idosos"  from( select id_oferta,sum(case when events='offer viewed' then 1 else 0 end) as "vistas",
...
-- 3100 pessoas abaixo de 40, 5850 abaixo de 60, 5875 acima de 60 tirando o erro

-- porcentagem de pessoas por faixa salarial, quando as ofertasa foram visualizadas e completadas
select portfolio.oferta, 100*sum(case when (p.salário<=50000 ) then 1 else 0 end)/count(p.salário) as "faixa salarial 1",
 100*sum(case when (p.salário >50000 and p.salário<=70000) then 1 else 0 end)/count(p.salário) as "faixa salarial 2",
 100*sum(case when (p.salário >70000 and p.salário<=90000) then 1 else 0 end)/count(p.salário) as "faixa salarial 3",
 100*sum(case when (p.salário >90000 and p.salário<=110000) then 1 else 0 end)/count(p.salário) as "faixa salarial 4",
